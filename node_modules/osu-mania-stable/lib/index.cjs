...
class SpecificBeatmapPatternGenerator extends PatternGenerator {
  *generate() {
    ...
    if (this.hitObject.hitType & osuClasses.HitType.Hold) {
      const hold = new Hold();
      const posData = this.hitObject;
...
class StageDefinition {
  ...
  getTypeOfColumn(column) {
    if (this.isSpecialColumn(column)) {
      return exports.ColumnType.Special;
    }

    const distanceToEdge = Math.min(column, this.columns - 1 - column);

    return distanceToEdge % 2 ? exports.ColumnType.Even : exports.ColumnType.Odd;
  }
  clone() {
...
class ManiaBeatmapConverter extends osuClasses.BeatmapConverter {
  constructor() {
    super(...arguments);
    ...
    this._rng = null;
    this._prevNoteTimes = [];
    this._density = osuClasses.FastRandom.MAX_INT32;
    this._lastTime = 0;
    this._lastPosition = new osuClasses.Vector2(0, 0);
    this._lastPattern = new Pattern();
    this._lastStair = exports.PatternType.Stair;
  }
  canConvert(beatmap) {
  ...
  convertBeatmap(original) {
    ...
    let seed = osuClasses.RoundHelper.round(difficulty.approachRate);

    seed += osuClasses.RoundHelper.round(difficulty.drainRate + difficulty.circleSize) * 20;
    seed += Math.trunc(difficulty.overallDifficulty * 41.2);
    this._rng = new osuClasses.FastRandom(Math.trunc(seed));
  ...
  *_generateConverted(hitObject, beatmap) {
    ...
    if (hitObject.path) {
      ...
      for (let i = 0; i <= generator.spanCount; ++i) {
        ...
        this._recordNote(time, position || new osuClasses.Vector2(0, 0));
      ...
      this._recordNote(spinner.endTime, new osuClasses.Vector2(256, 192));
  ...
  _updateTargetColumns(original) {
    ...
    const roundedCS = osuClasses.RoundHelper.round(difficulty.circleSize);
    const roundedOD = osuClasses.RoundHelper.round(difficulty.overallDifficulty);
...
class ManiaBeatmapProcessor extends osuClasses.BeatmapProcessor {
}

class ManiaDifficultyAttributes extends osuClasses.DifficultyAttributes {
  constructor() {
    super(...arguments);
...
class ManiaPerformanceAttributes extends osuClasses.PerformanceAttributes {
...
class ManiaDifficultyHitObject extends osuClasses.DifficultyHitObject {
...
class Strain extends osuClasses.StrainDecaySkill {
...
class ManiaDifficultyCalculator extends osuClasses.DifficultyCalculator {
  ...
  _createDifficultyHitObjects(beatmap, clockRate) {
    ...
    const comparerFn = (a, b) => {
      return osuClasses.RoundHelper.round(a.startTime) - osuClasses.RoundHelper.round(b.startTime);
    };

    const sortedObjects = osuClasses.SortHelper.depthSort(hitObjects, comparerFn);
  ...
  _getHitWindow300(mods) {
    const applyModAdjustments = (value) => {
      if (mods.has(osuClasses.ModBitwise.HardRock)) {
        return value / 1.4;
      }

      if (mods.has(osuClasses.ModBitwise.Easy)) {
        return value * 1.4;
      }
    ...
    if (osuClasses.RoundHelper.round(this._originalOverallDifficulty) > 4) {
...
class ManiaPerformanceCalculator extends osuClasses.PerformanceCalculator {
  ...
  _addParams(attributes, score) {
    ...
    if (score) {
      ...
      this._countPerfect = (_b = score.statistics.get(osuClasses.HitResult.Perfect)) !== null && _b !== void 0 ? _b : 0;
      this._countGreat = (_c = score.statistics.get(osuClasses.HitResult.Great)) !== null && _c !== void 0 ? _c : 0;
      this._countGood = (_d = score.statistics.get(osuClasses.HitResult.Good)) !== null && _d !== void 0 ? _d : 0;
      this._countOk = (_e = score.statistics.get(osuClasses.HitResult.Ok)) !== null && _e !== void 0 ? _e : 0;
      this._countMeh = (_f = score.statistics.get(osuClasses.HitResult.Meh)) !== null && _f !== void 0 ? _f : 0;
      this._countMiss = (_g = score.statistics.get(osuClasses.HitResult.Miss)) !== null && _g !== void 0 ? _g : 0;
    }
...
})(exports.ManiaAction || (exports.ManiaAction = {}));

class ManiaReplayFrame extends osuClasses.ReplayFrame {
  constructor() {
    super(...arguments);
  ...
  fromLegacy(currentFrame, _, beatmap) {
    ...
    let normalAction = exports.ManiaAction.Key1;
    let specialAction = exports.ManiaAction.Special1;
  ...
  toLegacy(beatmap) {
    ...
    for (const action of this.actions) {
      if (action === exports.ManiaAction.Special1) {
        keys |= 1 << this._getSpecialColumnIndex(maniaBeatmap, 0);
        continue;
      }

      if (action === exports.ManiaAction.Special2) {
        keys |= 1 << this._getSpecialColumnIndex(maniaBeatmap, 1);
        continue;
      }

      let nonSpecialKeyIndex = action - exports.ManiaAction.Key1;
      let overallIndex = 0;
    ...
    return new osuClasses.LegacyReplayFrame(this.startTime, this.interval, new osuClasses.Vector2(keys, 0), osuClasses.ReplayButtonState.None);
  }
...
class ManiaReplayConverter extends osuClasses.ReplayConverter {
...
class ManiaHitWindows extends osuClasses.HitWindows {
  isHitResultAllowed(result) {
    switch (result) {
      case osuClasses.HitResult.Perfect:
      case osuClasses.HitResult.Great:
      case osuClasses.HitResult.Good:
...
class ManiaRuleset extends osuClasses.Ruleset {
  get id() {
    return 3;
  }
  applyToBeatmap(beatmap) {
...
exports.DistanceObjectPatternGenerator = DistanceObjectPatternGenerator;
exports.EndTimeObjectPatternGenerator = EndTimeObjectPatternGenerator;
exports.HitObjectPatternGenerator = HitObjectPatternGenerator;
exports.Hold = Hold;
exports.HoldHead = HoldHead;
exports.HoldTail = HoldTail;
exports.HoldTick = HoldTick;
exports.ManiaAutoplay = ManiaAutoplay;
exports.ManiaBeatmap = ManiaBeatmap;
exports.ManiaBeatmapConverter = ManiaBeatmapConverter;
exports.ManiaBeatmapProcessor = ManiaBeatmapProcessor;
exports.ManiaCinema = ManiaCinema;
exports.ManiaDifficultyAttributes = ManiaDifficultyAttributes;
exports.ManiaDifficultyCalculator = ManiaDifficultyCalculator;
exports.ManiaDifficultyHitObject = ManiaDifficultyHitObject;
exports.ManiaDoubleTime = ManiaDoubleTime;
exports.ManiaDualStages = ManiaDualStages;
exports.ManiaEasy = ManiaEasy;
exports.ManiaEventGenerator = ManiaEventGenerator;
exports.ManiaFadeIn = ManiaFadeIn;
exports.ManiaFlashlight = ManiaFlashlight;
exports.ManiaHalfTime = ManiaHalfTime;
exports.ManiaHardRock = ManiaHardRock;
exports.ManiaHidden = ManiaHidden;
exports.ManiaHitObject = ManiaHitObject;
exports.ManiaHitWindows = ManiaHitWindows;
exports.ManiaKey1 = ManiaKey1;
exports.ManiaKey2 = ManiaKey2;
exports.ManiaKey3 = ManiaKey3;
exports.ManiaKey4 = ManiaKey4;
exports.ManiaKey5 = ManiaKey5;
exports.ManiaKey6 = ManiaKey6;
exports.ManiaKey7 = ManiaKey7;
exports.ManiaKey8 = ManiaKey8;
exports.ManiaKey9 = ManiaKey9;
exports.ManiaKeyMod = ManiaKeyMod;
exports.ManiaMirror = ManiaMirror;
exports.ManiaModCombination = ManiaModCombination;
exports.ManiaNightcore = ManiaNightcore;
exports.ManiaNoFail = ManiaNoFail;
exports.ManiaNoMod = ManiaNoMod;
exports.ManiaPerfect = ManiaPerfect;
exports.ManiaPerformanceAttributes = ManiaPerformanceAttributes;
exports.ManiaPerformanceCalculator = ManiaPerformanceCalculator;
exports.ManiaRandom = ManiaRandom;
exports.ManiaReplayConverter = ManiaReplayConverter;
exports.ManiaReplayFrame = ManiaReplayFrame;
exports.ManiaRuleset = ManiaRuleset;
exports.ManiaSuddenDeath = ManiaSuddenDeath;
exports.Note = Note;
exports.Pattern = Pattern;
exports.PatternGenerator = PatternGenerator;
exports.SpecificBeatmapPatternGenerator = SpecificBeatmapPatternGenerator;
exports.StageDefinition = StageDefinition;
